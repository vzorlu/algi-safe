...
(function () {
  ...
  if (layerControlVar) {
    ...
    const map = L.map('layerControl', {
      ...
      minZoom: 3,
      zoomControl: true
    }).setView([41.0082, 28.9784], 10); // Istanbul coordinates
    ...
    // Fetch device locations from the correct endpoint
    fetch('/api/stream/device-locations/') // Updated URL
      .then(response => response.json())
      .then(data => {
        console.log('Device locations:', data); // Debug log
        data.forEach(location => {
          const marker = L.marker([location.lang, location.long]).addTo(map).bindPopup(`
              <div style="color: #fff;">
                <strong>Device ID:</strong> ${location.deviceuuid}<br>
                <strong>Address:</strong> ${location.fulladdress || 'N/A'}<br>
        ...
        if (data.length > 0) {
          const bounds = L.latLngBounds(data.map(loc => [loc.lang, loc.long]));
